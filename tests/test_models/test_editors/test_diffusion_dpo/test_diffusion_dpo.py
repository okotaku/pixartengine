...
class TestDiffusionDPO(TestCase):
    ...
    def test_train_step(self):
        ...
        # test train step
        data = dict(
            inputs=dict(img=[torch.zeros((6, 64, 64))], text=["a dog"]))
    ...
    def test_train_step_v_prediction(self):
        ...
        optim_wrapper = OptimWrapper(optimizer)
        with pytest.raises(
                NotImplementedError, match="v_prediction is not implemented"):
